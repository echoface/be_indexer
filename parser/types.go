...
type (
	// ValueTokenizer converts values to string lists for index term construction.
	// Supports bidirectional parsing: indexing phase and query phase may differ.
	ValueTokenizer interface {
		// TokenizeValue parses a boolean expression value (e.g., "30:90:1000" → multiple geohashes).
		TokenizeValue(v interface{}) ([]string, error)

		// TokenizeAssign parses query parameters (e.g., [30.5, 98.2] → single geohash).
